...
- **現在の歌詞を任意でフローティング表示**：Spotify内の別ページを見ている間も、再生中の一行とふりがなを移動できるカードに表示します。
...
- 透明度を40%〜100%に調整
- 上下の間隔を最大8 px追加
- 現在の歌詞とふりがなをSpotify内の移動できるフローティングカードに表示
- ワンクリックで表示設定を初期値に戻す
- 実験的なオンライン高精度読みを有効化し、ローカルキャッシュを消去

設定はローカルに保存され、すぐに反映されます。

フローティングカードはSpotifyデスクトップ版が元から取得する行同期歌詞を使用し、Spotifyウィンドウ内だけに表示されます。オンライン高精度読みを別途有効にしない限り、追加の歌詞サービスには接続しません。
