...
- **可选当前句悬浮显示**：在浏览 Spotify 其他页面时，用窗口内可拖动卡片持续显示正在唱的歌词及其 Furigana。
...
- 将透明度调整为 40%–100%；
- 增加最多 8 px 的上下间距；
- 在 Spotify 窗口内用可拖动悬浮卡片显示当前歌词及其 Furigana；
- 一键恢复默认显示。
- 开启实验性的在线精准读音，并随时清除其本地缓存。

设置会保存在本地并立即生效。

悬浮卡片使用 Spotify 桌面端原本就会读取的行级同步歌词，并且只显示在 Spotify 窗口内。除非你另外开启在线精准读音，否则它不会联系额外的歌词服务。
