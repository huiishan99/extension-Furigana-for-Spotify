...
| --- | --- | --- | --- | --- | --- |
| 2026-08-27 | Windows 11 Pro 10.0.26200 | Microsoft Store 1.2.97.270 | 2.44.0 | active DOM line + Spotify line timestamps | Pass (v0.6.0 floating current lyric across routes) |
...
The v0.6.0 check installed the packaged preview and enabled the multilingual floating-lyric setting. On the lyrics page, the card exactly matched the active line's HTML and ruby count. On Spotify Home, where no active lyric element existed, the card matched the current line returned by Spotify's authenticated line-synced lyrics endpoint; seeking forward 30 seconds advanced the card to the new timestamped line with furigana intact. Synthetic drag input moved the card by the expected delta, persisted its coordinates, and restored the same position after the card was removed and recreated. Spotify was left paused with normal launch flags restored and the debugging port closed.
