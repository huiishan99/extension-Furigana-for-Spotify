...
- 新增默认关闭的“当前句悬浮显示”：歌词页直接复制 Spotify 当前活动行及其已生成的 `<ruby>` Furigana；浏览其他页面时改用 Spotify 自身的行级时间戳与播放进度，不猜测音频时间轴。
- 悬浮卡片位于 Spotify 窗口内部，可拖动并记住位置；切歌、路由切换和 Spotify 替换 `<body>` 后会重新挂载，没有同步歌词时自动隐藏，关闭功能后立即移除。
- 中日英设置页和用户文档新增开关与使用说明；观察器开始监听当前句的 class/ARIA 状态变化，新增活动行选择、Spotify 同步歌词解析、进度查找、位置恢复及边界约束测试。
- 在 Spotify 1.2.97.270 + Spicetify 2.44.0 实装验证：歌词页 49/58 行完成 109 个 ruby 注音，悬浮卡片与活动行 HTML 完全一致；离开歌词页后，在无活动歌词 DOM 的首页仍与 Spotify 当前时间戳行一致，跳播 30 秒后自动换句并保留 Furigana；拖动坐标在重建后正确恢复。
- 版本提升至 `0.6.0`；本版本不引入后台常驻进程，系统级跨应用置顶歌词留作独立功能评估。
