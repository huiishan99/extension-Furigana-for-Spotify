...
On Windows, this launcher also starts the optional native desktop-lyrics window. Enable **Floating current lyric** in the Spotify sidebar settings to show the synchronized line and furigana above other Windows apps. Lyric communication stays on the device, only the screen position is saved, and the window exits with Spotify. This desktop overlay is not available on macOS.
...
Windows 启动器还会启动可选的原生桌面歌词窗口。在 Spotify 侧边栏设置中打开 **当前句悬浮显示**，即可让同步歌词与 Furigana 置顶显示在其他 Windows 程序上方。歌词通信只在本机进行，只保存屏幕位置，并随 Spotify 一同退出；macOS 暂不提供这个桌面悬浮窗。
...
Windowsのランチャーは、任意のネイティブデスクトップ歌詞ウィンドウも起動します。Spotifyサイドバー設定の **現在の歌詞をフローティング表示** をオンにすると、同期歌詞とふりがながほかのWindowsアプリより前面に表示されます。歌詞通信は端末内だけで行い、画面上の位置だけを保存し、Spotifyと一緒に終了します。macOSではこのデスクトップオーバーレイを利用できません。
